...
| Object B text-to-3D with threestudio/SDS | script exists: `generate_text3d_threestudio.sh`; current artifact `results/task1/aigc/object_B_text3d/export/model.obj` is synthetic demo provenance | Pipeline prepared; current checked artifact is demo |
| Object C single image to 3D with Magic123 | script exists: `generate_image3d_magic123.sh`; current artifact `results/task1/aigc/object_C_image3d/model.obj` is synthetic demo provenance | Not complete for final grading until `data/raw/object_C/object_c.png` is provided and Magic123 is rerun |
